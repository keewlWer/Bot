...
def save_to_db(user_id: int) -> None:
    ...
    if len(user_data) == 8:
        ...
        c.execute("INSERT OR REPLACE INTO users VALUES (?,?,?,?,?,?,?,?)", tuple(user_data))
        conn.commit()
        conn.close()
    else:
...
def search_for_user(id: str) -> list:
    ...
    age_result = c.fetchone()
    if age_result is None:
        conn.close()
        return []

    user_age = int(age_result[0])
    users_for_search = []

    while not users_for_search and x <= max_age_difference:
        c.execute("SELECT * from users WHERE age <= ? AND age >= ? AND user_id != ?", (user_age + x, user_age - x, id))
